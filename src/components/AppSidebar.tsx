...
/>          </div>
          {!collapsed && <span className="text-lg font-bold tracking-wide">حصين كلينك</span>}
        </div>
      ...
      {/* Main */}
      <div className={cn("flex-1 transition-all duration-300", collapsed ? "mr-16" : "mr-64")}>
        {/* Top bar */}
        <header className="sticky top-0 z-30 h-14 bg-background/80 backdrop-blur-md border-b flex items-center px-6 gap-4">
          ...
          <h1 className="text-sm font-medium text-muted-foreground">
            {navItems.find((i) => i.path === location.pathname)?.label ?? "حصين كلينك"}
          </h1>
        </header>
